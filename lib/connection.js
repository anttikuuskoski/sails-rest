...
function runBeforeHooks(req, method, config, conn, cb){
  ...
  var hooks = conn.connection.hooks.before;
  //See if we have per-model overrides
  if(_.has(sails.models[conn.collection], ['tsrCallbacks', method, 'before']) &&
     Array.isArray(sails.models[conn.collection].tsrCallbacks[method]['before'])) {
      hooks = hooks.concat(
          sails.models[conn.collection].tsrCallbacks[method]['before']
      );
  }
    
  async.eachSeries(hooks, function (hook, nextHook) {
 ...
 */
function runAfterHooks(method, conn, err, res, cb){
  var hooks = conn.connection.hooks.after;
  //See if we have per-model overrides
  if(_.has(sails.models[conn.collection], ['tsrCallbacks', method, 'after']) &&
     Array.isArray(sails.models[conn.collection].tsrCallbacks[method]['after'])) {
      hooks = hooks.concat(
          sails.models[conn.collection].tsrCallbacks[method]['after']
      );
  }

  async.eachSeries(hooks, function (hook, nextHook) {
    hook(method, conn, err, res, nextHook);
  }, cb);
}
 ...
 */
function handleResponse(method, conn, err, res, cb) {
  runAfterHooks(method, conn, err, res, function (errFromHooks) {
    if(errFromHooks) {
 ...
 */
function getResponseHandler(method, conn, cb) {
  return function (err, res) {
    if (res && !err) {
      err = null;
    }
    handleResponse(method, conn, err, res, cb);
  }
}
...
Connection.find = function (connection, collection, options, cb) {
  ...
  runBeforeHooks(req, 'find', config, conn, function (err, req) {
    ...
    return req.query(conn.options).end(function (err, res) {
      ...
      handleResponse('find', conn, err, res, cb);
    });
  });
};
...
Connection.create = function (connection, collection, values, cb) {
  ...
  runBeforeHooks(req, 'create', config, conn, function (err, req) {
    ...
    return req.send(conn.values).end(getResponseHandler('create', conn, cb));
  });
};
...
Connection.update = function (connection, collection, options, values, cb) {
  ...
  runBeforeHooks(req, 'update', config, conn, function (err, req) {
    ...
    return req.query(conn.options).send(conn.values).end(getResponseHandler('update', conn, cb));
  });
};
...
Connection.destroy = function (connection, collection, options, cb) {
  ...
  runBeforeHooks(req, 'destroy', config, conn, function (err, req) {
    ...
    return req.query(conn.options).end(getResponseHandler('destroy', conn, cb));
  });
};
